  ...
  def index
    ...
    if @owner.present?
      ...
      if use_caching? && params[:work_search].blank? && params[:fandom_id].blank? &&
        ...
        @works = Rails.cache.fetch("#{@owner.works_index_cache_key(subtag)}_#{user}_page#{params[:page]}_true", expires_in: ArchiveConfig.SECONDS_UNTIL_WORK_INDEX_EXPIRE.seconds) do # i18n-locale-independent
      ...
      @works = Rails.cache.fetch('works/index/latest/v1', expires_in: ArchiveConfig.SECONDS_UNTIL_WORK_INDEX_EXPIRE.seconds) do # i18n-locale-independent
