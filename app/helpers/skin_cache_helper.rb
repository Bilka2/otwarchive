module SkinCacheHelper
  ...
  def skin_cache_version(skin_id)
    Rails.cache.fetch(skin_cache_version_key(skin_id)) do # i18n-locale-independent
      cache_timestamp
    end
  end

  def skin_cache_version_update(skin_id)
    Rails.cache.write(skin_cache_version_key(skin_id), cache_timestamp) # i18n-locale-independent
  end
